    ...
    private void OnTriggerEnter(Collider other)
    {
        ...
        else
        {
            if (other.gameObject.CompareTag("Enemy") || other.gameObject.CompareTag("Ground"))
            {
                ...
                //_pooledExplosion = GetPooledObject(_explosion, collisionPoint, other.transform.rotation);
                _pooledExplosion = GameManager.GetInstance().GetSpawner()._smallExplosionPool.GetPooledObject();
                ...
                _audioSource.Play();
                //ObjectPool.Singleton.ReturnPooledObject(this._pooledExplosion, GameManager.GetInstance().GetSpawner()._smallExplosionPrefab);
